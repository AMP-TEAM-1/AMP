...
## 🔐 보안 관련 env 이슈 ##
본 저장소의 초기 커밋 과정에서 .env 파일이 실수로 포함된 이력이 있습니다.
개발 초기 단계에서 환경 변수(.env)의 역할과 서버–클라이언트 연동 구조를 이해하기 위한 테스트 용도로 임시 값을 포함한 .env 파일을 사용했습니다.
이후 보안상 부적절하다는 점을 인지하여 즉시 .env 파일을 저장소에서 제거하였으며,
현재는 .env.example 파일만을 제공하고 실제 민감 정보는 포함되어 있지 않습니다.
