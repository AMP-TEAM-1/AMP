...
## 🔐 env 관련 보안 이슈 ##
본 프로젝트에는 실행을 위해 데이터베이스 연결 정보 및 서버 설정 값이 필요합니다.
일반적으로 .env 파일은 민감한 정보가 포함되므로 GitHub에 업로드하지 않는 것이 원칙입니다.
그러나 본 저장소는 대학교 프로젝트 평가 목적으로, 평가자가 별도의 환경 설정 없이 프로젝트를 직접 실행 및 검증할 수 있도록 하기 위해 예외적으로 .env 파일을 포함하여 업로드하였습니다.
